# Review package: Task 2 (git-free — manual file snapshot)

## Files created (both new)
- scripts/extract-wordlist.ts (new)
- scripts/__tests__/extract-wordlist.test.ts (new)

## Full contents: scripts/extract-wordlist.ts
```ts
...
## Full contents: scripts/__tests__/extract-wordlist.test.ts
```ts
...
## Test run output (reviewer: do not re-run, this is the implementer's evidence)
```
✔ parses a simple noun entry with a plural marker (6.7406ms)
✔ filters page-break junk lines (0.5174ms)
✔ keeps danken, der Dank and danke as separate flat entries (0.6956ms)
✔ keeps two independent example sentences as separate array items (0.6052ms)
✔ joins a line-wrapped example that has no terminating punctuation yet (2.6101ms)
✔ flags a capitalized noun line missing its article as a warning (0.4785ms)
ℹ tests 6
ℹ suites 0
ℹ pass 6
ℹ fail 0
